  ...
  try {
    ...
    const uploadedUrl = await Promise.all(
      files.map(async (file, index) => {
        ...
        const filePath = `${section}/nutrition/${id}/${fileName}`;
    ...
    console.error("Error to upload nutrition image", error);
  }
};

export const addNutritionApi = async (section, formData) => {
  try {
    const id = uuidv4();
    console.log(section);
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      console.log("Error to add nutrition", error);
      toast.error("Error to add nutrition");
      return;
    }
    toast.success("Nutrition added Successfully");
    return data;
  } catch (error) {
    toast.error("Error to adding nutrition");
    console.error("Error to Adding nutrition ", error);
  }
};
...
export const deleteNutritionApi = async (id) => {
  try {
    ...
    if (!deleting_data) {
      console.warn("⚠️ Nutrition not found, skipping deletion.");
    }
    console.log(deleting_data);

    let imageUrls = deleting_data.image_urls;
    if (Array.isArray(imageUrls)) {
      const deletePaths = imageUrls.map((item) => item.split("/sections/")[1]);
      console.log("Delete Path", deletePaths);
      ...
      if (storageError) {
        console.warn(
          "⚠️ Some images may not have been deleted:",
    ...
    toast.error("Error deleting Nutrition");
...
export const updateNutritionApi = async (section, formData) => {
  try {
    console.log(section);
    console.log(formData);
    ...
    const nutritionId = formData.id;
    console.log(nutritionId);
    const nutritionImages = formData.files;
    ...
    if (nutritionImages.some((item) => item instanceof File)) {
    ...
    if (error) {
      console.log("Error to update nutrition", error);
      toast.error("Error to update nutrition");
      return;
    }
    toast.success("nutrition updated Successfully");
    return data;
  } catch (error) {
    console.error("Error to update nutrition:", error);
  }
};

export const deleteNutritionImageAtIndex = async (rowId, indexToRemove) => {
  try {
